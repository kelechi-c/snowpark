...
class LlamaAttention(nnx.Module):
    def __init__(
        self, config: smollm_tiny_config, layer_id=None, qkv_bias=False, drop=0.0
    ):
        super().__init__()
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if position_embeddings is None:
            cos, sin = self.rotary_emb(key_states, query_states, position_ids).value
        else:
...
class Llama(nnx.Module):
    def __init__(self, config=smollm_tiny_config):
        ...
        self.embed_tokens = nnx.Embed(
            config.vocab_size, config.hidden_size, self.padding_idx, rngs=rngs
        )
        self.layers = [
